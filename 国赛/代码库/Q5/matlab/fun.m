    ...
    for i = 1:2:5 % 1 3 5
        ...
        if i <= 3  % 只对前两对检查，避免访问x(7)
            ...
            if abs(t_post - t_prev) < 1
                ...
                fprintf('违反约束2：|%f - %f| < 1 ', t_post, t_prev);
            end
        end
    ...
    for missile_idx = 1:3
        ...
        if ~isempty(current_intervals{missile_idx})
            if isempty(global_smoke_intervals{missile_idx})
                ...
                % 只提取时间列（前两列）进行合并
                global_time_intervals = global_smoke_intervals{missile_idx}(:, 1:2);
                combined_intervals = [global_time_intervals; current_intervals{missile_idx}];
