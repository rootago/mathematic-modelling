...
for i = 1:3
    global_smoke_intervals{i} = []; % 存储[开始时间, 结束时间, 无人机编号, 烟雾弹编号]
end
...
% 单架无人机的参数 - 根据物理约束重新设计边界
x0_single = [3.0, 5.0, 3.0, 5.0, 3.0, 5.0, 105, pi];      % 合理初值
% % [t1_1, t2_1, t1_2, t2_2, t1_3, t2_3, v, theta]
% lb_single = [0.5, 2.0, 0.5, 2.0, 0.5, 2.0, 70, 0];        % 下界
% ub_single = [15.0, 20.0, 15.0, 20.0, 15.0, 20.0, 140, 2*pi]; % 上界 

lb_single = [0, 0, 0, 0, 0,0 70, 0];        % 下界
ub_single = [20, 20, 20, 20, 20, 20, 140, 2*pi]; % 上界 

% fy1_x0_single = [4.76, 7.82, 7.82, 11.69, 11.69, 13.50, 140, 3.140732038];
% fy2_x0_single = [15.57, 17.85, 17.85, 21.07,21.07, 21.08,140,5.594006064];
% fy3_x0_single = [22.54, 27.65, 28.53, 32.80,32.80, 36.64,140,1.369491639];
% fy4_x0_single = [48.80, 66.00, 66.19, 66.20,66.37, 66.40,140,3.376021901];
% fy5_x0_single = [14.57, 17.43, 17.43, 21.60,21.60, 24.83,140,1.68133892];
    ...
    for i = 1:3
        if ~isempty(global_smoke_intervals{i})
            ...
            for j = 1:size(global_smoke_intervals{i}, 1)
                fprintf('[%.2f, %.2f](无人机%d-烟雾弹%d) ', ...
                    global_smoke_intervals{i}(j,1), global_smoke_intervals{i}(j,2), ...
                    global_smoke_intervals{i}(j,3), global_smoke_intervals{i}(j,4));
            end
            fprintf('\n');
        else
            fprintf('导弹%d现有区间：无\n', i);
...
for i = 1:3
    ...
    if ~isempty(global_smoke_intervals{i})
        ...
        fprintf('区间详情：\n');
        for j = 1:size(global_smoke_intervals{i}, 1)
            start_time = global_smoke_intervals{i}(j,1);
            end_time = global_smoke_intervals{i}(j,2);
            drone_id = global_smoke_intervals{i}(j,3);
            smoke_id = global_smoke_intervals{i}(j,4);
            interval_time = end_time - start_time;
            fprintf('  [%.2f, %.2f](%.2f秒) - 无人机%d的烟雾弹%d\n', ...
                start_time, end_time, interval_time, drone_id, smoke_id);
            total_time_missile = total_time_missile + interval_time;
        end
        fprintf('导弹%d总遮蔽时间：%.4f秒\n', i, total_time_missile);
