...
if t_start < t_end
 % 存储[开始时间, 结束时间, 无人机编号, 烟雾弹编号]
 current_intervals{j} = [current_intervals{j}; [t_start, t_end, curr_b, j]];
end
end
...
if isempty(global_smoke_intervals{missile_idx})
 global_smoke_intervals{missile_idx} = current_intervals{missile_idx};
else
 % 直接追加新的区间，不合并（保留详细信息）
 global_smoke_intervals{missile_idx} = [global_smoke_intervals{missile_idx}; current_intervals{missile_idx}];
 % 按开始时间排序
 global_smoke_intervals{missile_idx} = sortrows(global_smoke_intervals{missile_idx}, 1);
end
