...
class MatlabDebug:
    ...
    def get_time_exact_matlab(self, t1, t2, v, theta, APos0, BPos0):
        """完全按照MATLAB的getTime函数，添加详细调试"""
        print(f"\n=== getTime调试 ===")
        print(f"输入参数: t1={t1}, t2={t2}, v={v}, theta={theta}")
        print(f"APos0={APos0}, BPos0={BPos0}")
        ...
        print(f"alpha1={alpha1:.6f}, ADir={ADir}")
        ...
        for i in range(len(points)):
            ...
            print(f"开始仿真，时间步长={t_step}")
            
            step_count = 0
            while t <= 20 and step_count < 50:  # 限制步数用于调试
                # 计算导弹位置
                ...
                if step_count < 10:  # 只打印前几步
                    print(f"  t={t:.1f}: APos={APos}, DPos={DPos}")
                    print(f"    OA={OA}, OD={OD}")
                ...
                if flag1 == 0 and distance1 <= 10:
                    ...
                    print(f'  开始遮蔽: t={t:.1f}, distance1={distance1:.2f}, t_start={current_t_start:.2f}')
                    
                if flag1 == 1 and distance1 > 10:
                    current_t_end = t + t1 + t2
                    print(f'  结束遮蔽: t={t:.1f}, distance1={distance1:.2f}, t_end={current_t_end:.2f}')
                    break
                ...
                if flag2 == 1 and distance2 > 10:
                    current_t_end = t + t1 + t2
                    print(f'  距离2遮蔽结束: t={t:.1f}, distance2={distance2:.2f}, t_end={current_t_end:.2f}')
                    break
    ...
    def test_matlab_parameters(self):
        """测试MATLAB中的参数"""
        print("=== 测试MATLAB参数 ===")
        
        # 使用MATLAB第5架无人机的参数
        t1, t2, v, theta = 4.6633, 2.0281, 92.6482, 1.2617
        APos0 = self.APos0[0]  # 导弹1
        BPos0 = self.BPos0[4]  # 无人机5
        
        print(f"测试参数: t1={t1}, t2={t2}, v={v}, theta={theta}")
        print(f"导弹1位置: {APos0}")
        print(f"无人机5位置: {BPos0}")
...
def main():
    ...
    # 测试MATLAB参数
    debug.test_matlab_parameters()
    
    # 测试不同参数
    debug.test_different_parameters()
