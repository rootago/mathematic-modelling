function [x1,x2] = eq1(t1,t2,v,theta)
    ...
    OA1 = APos1 - OPos;
    OD1 = DPos1 - OPos;
    distance1 = norm(cross(OA1,OD1))/norm(OA1);
    
    % 计算x2对应的距离
    OA2 = APos2 - OPos;
    OD2 = DPos2 - OPos;
    distance2 = norm(cross(OA2,OD2))/norm(OA2);

    Objective = x1 + x2;
    Constraints = [];

    % 约束条件
    Constraints = [Constraints, x1 <= x2];
    Constraints = [Constraints, distance2 <= 10]; 
