...
sum_effect = T_effect + H_effect + SC_effect
T_effect = T_effect / sum_effect
H_effect = H_effect / sum_effect
SC_effect = SC_effect / sum_effect
def P_sat(T): # Pa 
...
for run in range(1):
    ...
    def objective(theta):
        ...
        for _,row in data.iterrows():
            T,H,SC,_ = row
            try:
                Pp = calculate_Pp((T+273.15)*T_effect,H*H_effect,SC/100*SC_effect,theta)
                Pp_pred.append(Pp)
            except:
    ...
    for _, row in data.iterrows():
        T, H, SC, _ = row
        final_pred.append(calculate_Pp((T+273.15)*T_effect, H*H_effect, SC/100*SC_effect, optimization_history['best_params']))
    ...
    results_df = pd.DataFrame({
        ...
        # 'theta': optimization_history['best_params']
    })
    print( optimization_history['best_params'])
...
for i, history in enumerate(optimization_histories):
    ...
    for _, row in data.iterrows():
        T, H, SC, _ = row
        final_pred.append(calculate_Pp((T+273.15)*T_effect, H*H_effect, SC/100*SC_effect, history['best_params']))
