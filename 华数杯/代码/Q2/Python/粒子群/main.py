...
def objective(X):
    ...
    for x in X:
        T, H, SC = x
        val = -calculate_Pp(T, H, SC)  # 自定义的计算函数
        results.append(val)
    return np.array(results)
# 变量边界
bounds = (np.array([30.+273.15, 50, 0]),    # 下界
          np.array([50+273.15, 90, 0.1]))    # 上界
# bounds = (np.array([30.+273.15, 0.2, 0]),    # 下界
#           np.array([50+273.15, 0.9, 0.2]))    # 上界

# 创建优化器实例，设定粒子数和维度
options = {'c1': 0.5, 'c2': 0.3, 'w': 0.9}  # 参数可调
optimizer = ps.single.GlobalBestPSO(n_particles=100, dimensions=3, options=options, bounds=bounds)

# 运行优化，迭代100次
best_cost, best_pos = optimizer.optimize(objective, iters=1000)

print("最优参数:", best_pos)
print("对应目标值:", -best_cost)