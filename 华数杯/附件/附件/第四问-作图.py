...
results_df = pd.read_excel("孔隙率计算结果2.xlsx")
...
cbar = fig1.colorbar(scatter, ax=ax1, pad=0.1)
cbar.set_label('Porosity (Pp)', fontsize=12)

ax1.set_xlabel('Temperature (°C)', fontsize=12, labelpad=10)
ax1.set_ylabel('Humidity (%)', fontsize=12, labelpad=10)
ax1.set_zlabel('SC (%)', fontsize=12, labelpad=10)
ax1.set_title('Porosity Distribution', fontsize=14)
...
sc_value = 0.08
sc_idx = np.abs(SC_values - sc_value).argmin()

fig2 = plt.figure(figsize=(10, 8))
ax2 = fig2.add_subplot(111, projection='3d')

sc = ax2.scatter(
    T_grid[:, :, sc_idx].flatten(), 
    H_grid[:, :, sc_idx].flatten(), 
    SC_grid[:, :, sc_idx].flatten() * 100,
    c=Pp_grid[:, :, sc_idx].flatten(), 
    cmap=cmap,
    s=50,
    alpha=0.7,
    edgecolor='w',
...
ax2.set_ylabel('Humidity (%)', fontsize=12, labelpad=10)
ax2.set_zlabel('SC (%)', fontsize=12, labelpad=10)
ax2.set_title(f'Porosity at SC = {sc_value*100:.1f}%', fontsize=14)
ax2.view_init(elev=25, azim=-45)
ax2.grid(True, linestyle='--', alpha=0.5)

plt.tight_layout()
plt.savefig('porosity_3d_fixed_sc.png', dpi=300, bbox_inches='tight')
plt.show()
