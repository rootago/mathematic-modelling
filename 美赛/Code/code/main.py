...
赛题要求1：使用连续时间方程表示荷电状态（SOC）；数据作为支持（参数估计与验证），非替代；非纯离散拟合/黑箱。
...
- 拟合：最小二乘 min sum (U_obs - (Vocv(SOC)-I*r))^2，得 k,A,B。
...
if not os.path.exists(DYNAMIC_DATA_PATH):
    ...
    if t_fit is None:
        ...
        S_fit = S_fit[::step]
        print(f"联合拟合数据点数: {len(t_fit)}, 时间范围: {t_fit[0]:.1f} ~ {t_fit[-1]:.1f} s")

        def residual(p):
            kv, Av, Bv = p[0], p[1], p[2]
            V = vocv(S_fit, E0, kv, Av, Bv, Q0)
            U_mod = V - I_fit * r
            return U_obs_fit - U_mod

        res = least_squares(residual, [0.15, 0.5, 1.0 / Q0], bounds=([1e-6, 1e-6, 1e-6], [2.0, 5.0, 10.0]), verbose=0)
        k, A, B = res.x
        # 用最优参数重新计算残差平方和（更严谨的做法）
        residuals_final = residual(res.x)
        true_cost = np.sum(residuals_final**2)
        rmse = np.sqrt(true_cost / len(t_fit))
        print(f"联合拟合结果: k = {k:.6f}, A = {A:.6f}, B = {B:.6f}")
        print(f"  残差平方和 RSS = {true_cost:.6f}")
        print(f"  均方根误差 RMSE = {rmse:.6f} V")
        print(f"  平均相对误差 = {rmse / np.mean(U_obs_fit) * 100:.2f}%")
        with open(os.path.join(OUTPUT_DIR, "vocv_fit_params.json"), "w", encoding="utf-8") as f:
            json.dump({
                "method": "Least Squares (scipy)",
                "E0": E0, 
                "k": k, 
                "A": A, 
                "B": B, 
                "Q0": Q0,
                "residual_sum_squares": float(true_cost)
            }, f, indent=2)
...
# ========== 3. 绘图 ==========
# ---------- 图1：fit_joint_U.png（全部来自「实测数据」DST 放电段）----------
...
if t_fit is not None:
    ...
    axes1[0].set_title("① 端电压：实测 vs 模型（拟合好坏看两条线是否接近）", fontsize=10)
    ...
    fig1.suptitle("图1：联合拟合结果（数据来源：data2 DST 放电段）", fontsize=12)
    fig1.tight_layout()
    fig1.savefig(os.path.join(OUTPUT_DIR, "fit_joint_U.png"), dpi=150, bbox_inches="tight")
    print(f"Figure saved: {OUTPUT_DIR}/fit_joint_U.png")

# ---------- 图2：battery_ode.png（全部来自「数值模拟」，不是实测）----------
...
ax.set_title("④ 开路电压 Vocv 与 SOC 的关系（拟合公式曲线）", fontsize=10)
ax.grid(True, alpha=0.5)
ax.set_facecolor("#fafafa")

fig2.suptitle("图2：ODE 数值模拟（起始 SOC=50%，恒功率 20W 放电 1h）", fontsize=12)
fig2.tight_layout()
fig2.savefig(os.path.join(OUTPUT_DIR, "battery_ode.png"), dpi=150, bbox_inches="tight")
print(f"Figure saved: {OUTPUT_DIR}/battery_ode.png")
