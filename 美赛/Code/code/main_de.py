"""
2026 MCM Problem A：智能手机电池消耗建模 — 使用差分进化算法拟合参数

与 main.py 的区别：使用差分进化算法（Differential Evolution, DE）代替最小二乘法拟合 k, A, B。
...
- 拟合：差分进化算法 min sum (U_obs - (Vocv(SOC)-I*r))^2，得 k,A,B。
...
if not os.path.exists(DYNAMIC_DATA_PATH):
    ...
    if t_fit is None:
        ...
        S_fit = S_fit[::step]
        print(f"差分进化算法拟合数据点数: {len(t_fit)}, 时间范围: {t_fit[0]:.1f} ~ {t_fit[-1]:.1f} s")
        ...
        # 参数边界
        bounds = [(1e-6, 2.0),    # k 的范围
                  (1e-6, 5.0),    # A 的范围
                  (1e-6, 10.0)]   # B 的范围

        print("开始差分进化优化...")
        print("差分进化（DE）是一种强大的全局优化算法，适合非线性、多峰问题。")
        
        # 使用 scipy 的差分进化算法
        result = differential_evolution(
            objective,
            bounds,
            strategy='best1bin',    # 变异策略：best1bin 是经典策略
            maxiter=1000,           # 最大迭代次数
            popsize=30,             # 种群大小（相对于参数维度的倍数）
            tol=1e-7,               # 收敛容差
            atol=1e-7,              # 绝对容差
            mutation=(0.5, 1.5),    # 变异常数范围
            recombination=0.7,      # 重组概率（交叉概率）
            seed=42,                # 随机种子
            callback=None,
            disp=True,              # 显示优化过程
            polish=True,            # 使用 L-BFGS-B 进行局部优化
            init='latinhypercube',  # 使用拉丁超立方采样初始化
            workers=1,
            updating='immediate'     # 立即更新策略
        )
        
        k, A, B = result.x
        final_cost = result.fun
        
        print(f"\n差分进化拟合结果: k = {k:.6f}, A = {A:.6f}, B = {B:.6f}, 残差平方和 = {final_cost:.6f}")
        print(f"优化状态: {result.message}")
        print(f"函数评估次数: {result.nfev}")
        print(f"迭代次数: {result.nit}")
        
        # 保存拟合参数
        with open(os.path.join(OUTPUT_DIR, "vocv_fit_params_de.json"), "w", encoding="utf-8") as f:
            json.dump({
...
# ========== 3. 绘图 ==========
# ---------- 图1：fit_joint_U_de.png（全部来自「实测数据」DST 放电段）----------
if t_fit is not None:
    ...
    axes1[0].set_title("① 端电压：实测 vs 模型（差分进化算法拟合）", fontsize=10)
    ...
    fig1.suptitle("图1：差分进化算法拟合结果（数据来源：data2 DST 放电段）", fontsize=12)
    fig1.tight_layout()
    fig1.savefig(os.path.join(OUTPUT_DIR, "fit_joint_U_de.png"), dpi=150, bbox_inches="tight")
    print(f"Figure saved: {OUTPUT_DIR}/fit_joint_U_de.png")

# ---------- 图2：battery_ode_de.png（全部来自「数值模拟」，不是实测）----------
...
ax.set_ylim(0, 1.05)
ax.set_title("① 模拟的 SOC 随时间（从 50% 起恒功率放电）", fontsize=10)
...
ax.set_title("④ 开路电压 Vocv 与 SOC 的关系（差分进化拟合曲线）", fontsize=10)
ax.grid(True, alpha=0.5)
ax.set_facecolor("#fafafa")

fig2.suptitle("图2：ODE 数值模拟（起始 SOC=50%，恒功率 20W 放电 1h，差分进化参数）", fontsize=12)
fig2.tight_layout()
fig2.savefig(os.path.join(OUTPUT_DIR, "battery_ode_de.png"), dpi=150, bbox_inches="tight")
print(f"Figure saved: {OUTPUT_DIR}/battery_ode_de.png")
