...
功率公式（从实测数据拟合，含截距 + 非线性项）：
  P_total = C0 + P_screen + P_cpu + P_network + P_gps
  C0: 截距/背景功耗（待机、后台等）
  P_screen = screen × (C1 + C2×bright + C8×bright²)   # 亮度二次项
  P_cpu = C3×(cpu×freq) + C9×(cpu×freq)²             # CPU 二次项
  P_net = C4 × throughput + C5 × exp(-λ × wifi_intensity)
  P_gps = C6 × gps_status + C7 × (gps_status × gps_activity²)
...
def fit_power_model(data):
    ...
    # 含截距 C0 + 线性项 + 非线性项(亮度², CPU²)
    coef_names = ['C0_intercept', 'C1_screen', 'C2_bright', 'C3_cpu_usage_freq', 'C4_throughput', 
                  'C5_wifi', 'C6_gps', 'C7_gps_quad', 'C8_bright_quad', 'C9_cpu_quad']
    ...
    for source_file, group_df in data.groupby('source_file'):
        ...
        # 非线性项：亮度² (屏亮时)
        df['x_bright_quad'] = df['screen_on'] * (df['bright_norm'] ** 2)
        ...
        # 非线性项：CPU²
        df['x_cpu_quad'] = df['x_cpu_usage_freq'] ** 2
        ...
        # 拟合：截距 + 所有特征（含非线性项）
        X_fit = np.hstack([np.ones((len(X), 1)), X])  # 第一列为截距
...
class PowerModel:
    def __init__(self, fit_result):
        self.params = fit_result['params']  # [C0, C1, C2, C3, C4, C5, C6, C7, C8, C9]
    ...
    def calc_power_from_row(self, row, return_components=False):
        ...
        x_screen_bright = screen_on * bright_norm
        x_bright_quad = screen_on * (bright_norm ** 2)
        ...
        features = [x_screen_base, x_screen_bright, cpu_usage_freq, throughput, wifi_exp, gps_on, gps_quad,
                    x_bright_quad, x_cpu_quad]
        
        if return_components:
            P_base = self.params[0]  # 截距
            P_screen = self.params[1] * x_screen_base + self.params[2] * x_screen_bright + self.params[8] * x_bright_quad
            P_cpu = self.params[3] * cpu_usage_freq + self.params[9] * x_cpu_quad
            ...
            P = self.params[0] + sum(p * f for p, f in zip(self.params[1:], features))
            return max(P, 0.1)
...
if data is not None:
    ...
    if fit_result is not None:
        ...
        with open(os.path.join(OUTPUT_DIR, "power_model_fitted.json"), "w", encoding="utf-8") as f:
            json.dump({
                "method": "per-experiment fitting with weighted average (intercept + nonlinear: bright², cpu²)",
