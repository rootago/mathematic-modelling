...
电池模型（二阶 RC 极化模型 + 老化）：
  ODE 方程组:
    ...
    dUp1/dt = -Up1 / (R1×C1) + I / C1     # 第一个极化电压动态
    dUp2/dt = -Up2 / (R2×C2) + I / C2     # 第二个极化电压动态
    dF/dt   = -θ₁ × D                     # 衰减因子动态
  端电压:
    Um(t) = Vocv(S(t)) - I(t)×R0 - Up1(t) - Up2(t)
...
# ---------- 极化参数（二阶 RC 模型）----------
R1 = 0.03         # 第一个极化电阻 (Ω)
C1 = 1000.0       # 第一个极化电容 (F)，时间常数 τ1 = R1 * C1 = 30s
R2 = 0.02         # 第二个极化电阻 (Ω)
C2 = 500.0        # 第二个极化电容 (F)，时间常数 τ2 = R2 * C2 = 10s
...
def get_soc_curve_to_empty(segment, power_model, Q_est=None, theta1=1e-8, theta2=0.001):
    ...
    def odefun(t, y):
        ...
        V_eff = Vocv_val - Up1_val - Up2_val
        ...
        dUp1_dt = -Up1_val / (R1 * C1) + I_val / C1
        dUp2_dt = -Up2_val / (R2 * C2) + I_val / C2
        dF_dt = -theta1 * D
        
        return np.array([dS_dt, dUp1_dt, dUp2_dt, dF_dt])
    
    dt_median = np.median(np.diff(t_data)) if len(t_data) > 1 else 60
    max_step = min(max(dt_median * 2, 10), 60)
    
    sol = solve_ivp(odefun, t_span, [S0, Up10, Up20, F0], method='RK45', dense_output=True,
...
def plot_soc_correlation(original_soc, modified_soc, rmse_values):
    ...
    # 计算相关性统计
    correlation = np.corrcoef(original_soc, modified_soc)[0, 1]
    r_squared = correlation ** 2
    rmse_overall = np.sqrt(np.mean((modified_soc - original_soc)**2))
    
    # 输出R²统计信息（显眼格式）
    print("\n" + "="*70)
    print("【模型预测准确性分析】")
    print("="*70)
    print(f"  数据点总数: {len(original_soc):,}")
    print(f"  相关系数 (r): {correlation:.6f}")
    print(f"  ★ R² (决定系数): {r_squared:.6f}  ★")
    print(f"  总体RMSE: {rmse_overall:.4f}%")
    print(f"  预测准确度: {(1 - rmse_overall/100)*100:.2f}%")
    print("="*70)
    ...
    with plt.rc_context({"font.family": "Times New Roman"}):
        ...
        # 为了图形美观，进行降采样（数据点太多会很密集）
        n_total = len(original_soc)
        sample_rate = max(1, n_total // 5000)  # 最多显示5000个点
        indices = np.arange(0, n_total, sample_rate)
        
        # 绘制散点图
        ax.scatter(original_soc[indices], modified_soc[indices], 
                  alpha=0.3, s=12, c='#3498db', edgecolors='none',
                  label=f'Data points (n={len(indices):,})')
        
        # 绘制 y=x 参考线
        min_val = min(np.min(original_soc), np.min(modified_soc))
        max_val = max(np.max(original_soc), np.max(modified_soc))
        ax.plot([min_val, max_val], [min_val, max_val], 
               'r--', linewidth=3.0, alpha=0.8, label='y = x (Perfect prediction)')
        
        # 设置坐标轴（字体放大）
        ax.set_xlabel("Predicted SOC (%)", fontsize=18, fontweight='bold')
        ax.set_ylabel("Fluctuated SOC (%)", fontsize=18, fontweight='bold')
...
def validate_with_real_data(segment, power_model, Q_est=None, theta1=1e-8, theta2=0.001, T_override=None):
    ...
    def odefun(t, y):
        ...
        Up1_val = y[1]
        Up2_val = y[2]
        F_val = np.clip(y[3], 0.5, 1.0)  # F 限制在 [0.5, 1.0]，避免容量过度衰减
        ...
        # 求解电流 I：P = I * (Vocv - I * R0 - Up1 - Up2)
        # => R0 * I^2 - (Vocv - Up1 - Up2) * I + P = 0
        V_eff = Vocv_val - Up1_val - Up2_val
        discriminant = V_eff**2 - 4 * R0 * P
    ...
    sol = solve_ivp(odefun, t_span, [S0, Up10, Up20, F0], method='RK45', dense_output=True, max_step=max_step)
    
    # 在数据时间点评估
    sol_values = sol.sol(t_data)
    SOC_simulated = np.clip(sol_values[0], 0, 1)
    Up1_simulated = sol_values[1]
    Up2_simulated = sol_values[2]
    F_simulated = sol_values[3]  # 衰减因子
    ...
    sol_meas = solve_ivp(odefun_measured, t_span, [S0, Up10, Up20, F0], method='RK45', dense_output=True, max_step=max_step)
    ...
    # 计算总极化电压
    Up_total_simulated = Up1_simulated + Up2_simulated
    ...
    return {
        ...
        'Up1_simulated': Up1_simulated,
        'Up2_simulated': Up2_simulated,
        'Up_simulated': Up_total_simulated,  # 总极化电压（向后兼容）
...
if data is not None:
    ...
    if fit_result is not None:
        ...
        original_soc, modified_soc, rmse_values = plot_all_experiment_soc_curves_with_noise(fit_result, data)
        ...
        if segments:
            ...
            # ========== 自动校准容量 Q ==========
            E_measured_J = np.trapezoid(result['P_measured'], x=result['t_min']*60)
            ...
            # 极化电压诊断
            Up1_avg = np.mean(result['Up1_simulated'])
            Up1_max = np.max(result['Up1_simulated'])
            Up2_avg = np.mean(result['Up2_simulated'])
            Up2_max = np.max(result['Up2_simulated'])
            Up_total_avg = np.mean(result['Up_simulated'])
            Up_total_max = np.max(result['Up_simulated'])
            print(f"\n极化电压诊断（二阶RC模型）:")
            print(f"  Up1: 平均 {Up1_avg*1000:.1f} mV, 最大 {Up1_max*1000:.1f} mV (τ1={R1*C1:.1f}s)")
            print(f"  Up2: 平均 {Up2_avg*1000:.1f} mV, 最大 {Up2_max*1000:.1f} mV (τ2={R2*C2:.1f}s)")
            print(f"  总极化电压: 平均 {Up_total_avg*1000:.1f} mV, 最大 {Up_total_max*1000:.1f} mV")
            ...
            if segment_analysis:
                ...
                print(f"\n共8张老化动态图已保存至 {aging_dynamics_dir}")
                ...
                with plt.rc_context({"font.family": "Times New Roman"}):
                    ...
                    ax.set_title("Battery Aging Dynamics - All Components Comparison (0-100 min)\n(Solid: Real Temperature, Dashed: 45°C)", 
                    ...
                    fig.tight_layout()
                    summary_fig_path = os.path.join(aging_dynamics_dir, "aging_dynamics_summary_all_8_curves.png")
