...
功率公式（从实测数据拟合，无基础功耗）：
  ...
  P_cpu = C3 × (cpu_usage × freq_norm) + C8 × (cpu_usage × freq_norm)²  # 线性 + 二次项
...
def fit_power_model(data):
    ...
    coef_names = ['C1_screen', 'C2_bright', 'C3_cpu_usage_freq', 'C4_throughput', 'C5_wifi', 'C6_gps', 'C7_gps_quad', 'C8_cpu_quad']
    ...
    for source_file, group_df in data.groupby('source_file'):
        ...
        df['x_cpu_usage_freq_quad'] = df['x_cpu_usage_freq'] ** 2  # CPU 二次项
        ...
        feature_names = ['x_screen_base', 'x_screen_bright', 'x_cpu_usage_freq', 'x_cpu_usage_freq_quad',
...
class PowerModel:
    ...
    def calc_power_from_row(self, row, return_components=False):
        ...
        cpu_usage_freq_quad = cpu_usage_freq ** 2
        ...
        features = [screen_on, screen_on * bright_norm, cpu_usage_freq, cpu_usage_freq_quad,
                    throughput, wifi_exp, gps_on, gps_quad]
        
        if return_components:
            # 返回各组件功率（包含基础功耗）
            P_screen = self.params[0] * screen_on + self.params[1] * screen_on * bright_norm
            P_cpu = self.params[2] * cpu_usage_freq + self.params[3] * cpu_usage_freq_quad
            P_network = self.params[4] * throughput + self.params[5] * wifi_exp
            P_gps = self.params[6] * gps_on + self.params[7] * gps_quad
...
# ==================== 5.5 拟合老化参数 theta1、theta2、Q ====================
def fit_aging_parameters(segment, power_model, Q_est=None, Q_ref=None, verbose=True):
    """
    用最小二乘法拟合 theta1、theta2、Q
    
    返回:
        best_theta1, best_theta2, best_Q, best_rmse
    """
    Q_ref = Q_ref if Q_ref is not None else (Q_est if Q_est is not None else Q0)
    n_pts = len(segment['data'])
    
    def residual_params(params):
        theta1 = 10 ** params[0]
        theta2 = 10 ** params[1]
        Q_opt = params[2]
        try:
            result = validate_with_real_data(segment, power_model, Q_est=Q_opt, theta1=theta1, theta2=theta2)
            return result['SOC_simulated'] - result['SOC_measured']
        except Exception:
            return np.ones(n_pts) * 10.0
    
    x0 = np.array([-8.0, -3.0, Q_ref])
    Q_lo = max(5.5, Q_ref * 0.75)
    Q_hi = min(10.0, Q_ref * 1.1)
    bounds = (np.array([-10.0, -5.0, Q_lo]), np.array([-5.0, -1.0, Q_hi]))
    
    if verbose:
        print("\n最小二乘拟合 theta1、theta2、Q...")
    
    result_ls = least_squares(residual_params, x0, bounds=bounds, verbose=0)
    theta1_opt = 10 ** result_ls.x[0]
    theta2_opt = 10 ** result_ls.x[1]
    Q_opt = result_ls.x[2]
    
    result_val = validate_with_real_data(segment, power_model, Q_est=Q_opt, theta1=theta1_opt, theta2=theta2_opt)
    rmse_opt = result_val['rmse_pct']
    
    if verbose:
        print(f"  最优参数: θ₁={theta1_opt:.2e}, θ₂={theta2_opt:.2e}, Q={Q_opt:.3f} Ah, RMSE={rmse_opt:.2f}%")
    return theta1_opt, theta2_opt, Q_opt, rmse_opt
...
if data is not None:
    ...
    if fit_result is not None:
        ...
        if segments:
            ...
            # 尝试两种功率模型，选 RMSE 更低的
            pm_weighted = PowerModel(fit_result)
            seg_basename = os.path.basename(best_seg['source'])
            seg_specific = next((r for r in fit_result['experiment_results'] if r['source'] == seg_basename), None)
            
            best_rmse = np.inf
            best_result = None
            best_power_model = None
            best_theta1 = best_theta2 = best_Q = None
            
            candidates = [(pm_weighted, "加权平均")]
            if seg_specific is not None:
                candidates.append((PowerModel({'params': seg_specific['params'], 'intercept': seg_specific['intercept'], 'lambda': fit_result['lambda']}), "验证段所属实验"))
            print("\n比较功率模型 (选 RMSE 更低的):")
            for pm, label in candidates:
                th1, th2, Qo, rmse = fit_aging_parameters(best_seg, pm, Q_ref=Q0, verbose=(len(candidates)==1))
                if len(candidates) > 1:
                    print(f"  {label}: RMSE={rmse:.2f}%")
                if rmse < best_rmse:
                    best_rmse = rmse
                    best_theta1, best_theta2, best_Q = th1, th2, Qo
                    best_power_model = pm
                    best_result = validate_with_real_data(best_seg, pm, Q_est=Qo, theta1=th1, theta2=th2)
            
            theta1_opt, theta2_opt, Q_opt, rmse_opt = best_theta1, best_theta2, best_Q, best_rmse
            power_model = best_power_model
            result = best_result
            model_label = "验证段所属实验" if seg_specific is not None and best_power_model is not pm_weighted else "加权平均"
            print(f"  选用: {model_label}")
