...
# real -> mobile -> aging -> code -> Code (root, 含 data 文件夹)
...
def simplify_curve(t, y, tolerance=0.5):
    """
    简化曲线，去掉长段的平线
    ...
    参数:
        t: 时间数组
        y: 数值数组
        tolerance: 容差，当连续点的变化小于此值时认为是平线段
    ...
    # 保留的点的索引
    keep_indices = [0]  # 始终保留第一个点
    
    i = 0
    while i < len(y) - 1:
        # 找到连续的平线段
        j = i + 1
        while j < len(y) and abs(y[j] - y[i]) < tolerance:
            j += 1
        
        # 如果找到平线段(长度 > 2个点)
        if j - i > 2:
            # 只保留平线段的中点或每隔一段距离保留一个点
            segment_length = j - i
            if segment_length > 5:
                # 对于很长的平线段，每隔几个点保留一个
                step = max(2, segment_length // 3)
                for k in range(i + step, j, step):
                    keep_indices.append(k)
            # 保留平线段的结束点
            keep_indices.append(j - 1)
            i = j
        else:
            # 不是平线段，保留下一个点
    ...
    # 始终保留最后一个点
    if keep_indices[-1] != len(t) - 1:
        keep_indices.append(len(t) - 1)
    
    return t[keep_indices], y[keep_indices]
...
def get_soc_curve_to_empty(segment, power_model, Q_est=None, theta1=1e-8, theta2=0.001):
    ...
    # 简化曲线，去掉长段的平线
    t_simplified, soc_simplified = simplify_curve(t_sol / 60, soc_sol, tolerance=0.3)
    
    return t_simplified, soc_simplified
...
def plot_all_experiment_real_soc_curves(fit_result, data):
    """
    仿照图1的逻辑，使用真实数据画真实曲线（而非预测曲线）。
    在一张图上绘制所有实验的实测 SOC 下降曲线。
    """
    segments = find_discharge_segments(data, min_duration_sec=600, min_soc_drop=5)
    ...
    with plt.rc_context({"font.family": "Times New Roman"}):
        ...
        for i, res in enumerate(fit_result['experiment_results']):
            ...
            # 使用真实数据：时间(分钟) 和 battery_level(实测SOC)
            df = best_seg['data'].copy()
            df['timestamp'] = pd.to_datetime(df['timestamp'])
            df = df.sort_values('timestamp').reset_index(drop=True)
            t0 = df['timestamp'].iloc[0]
            t_min = (df['timestamp'] - t0).dt.total_seconds().values / 60
            soc_real = df['battery_level'].values
            
            # 简化曲线，去掉长段的平线
            t_min_simplified, soc_real_simplified = simplify_curve(t_min, soc_real, tolerance=0.3)

            label = f"Group {i+1}"
            ax.plot(t_min_simplified, soc_real_simplified, alpha=0.7, linewidth=1.5,
                    color=colors[i % 20], label=label)
            n_plotted += 1

        ax.set_xlabel("Time (min)", fontsize=14)
        ax.set_ylabel("SOC (%)", fontsize=14)
        ax.set_title("SOC Real Curves for All Experiments (Measured Data)", fontsize=16)
        ax.legend(bbox_to_anchor=(1.02, 1), loc='upper left', fontsize=10, ncol=2)
        ax.tick_params(axis='both', which='major', labelsize=12)
        ax.grid(True, alpha=0.4)
        ax.set_xlim(left=0)
        ax.set_ylim(0, 105)
        fig.tight_layout(rect=[0, 0, 0.85, 1])
        out_path = os.path.join(OUTPUT_DIR, "pic1_real.png")
        fig.savefig(out_path, dpi=150, bbox_inches="tight")
        print(f"\n所有实验真实 SOC 曲线已画到一张图: {out_path}（共 {n_plotted} 条曲线）")
        plt.close(fig)
...
if data is not None:
    ...
    if fit_result is not None:
        ...
        # 绘制所有实验真实 SOC 曲线（仿照图1逻辑，用实测数据）
        plot_all_experiment_real_soc_curves(fit_result, data)
        ...
        if segments:
            ...
            # 简化曲线
            t_measured_simp, soc_measured_simp = simplify_curve(result['t_min'], result['SOC_measured'], tolerance=0.3)
            t_simulated_simp, soc_simulated_simp = simplify_curve(result['t_min'], result['SOC_simulated'], tolerance=0.3)
            ax.plot(t_measured_simp, soc_measured_simp, 'b-', linewidth=2, label='Measured SOC')
            ax.plot(t_simulated_simp, soc_simulated_simp, 'r--', linewidth=2, label='Simulated SOC (with Aging)')
            ...
            fig, ax = plt.subplots(figsize=(6, 4))
            # 简化曲线
            t_f_simp, f_simp = simplify_curve(result['t_min'], result['F_simulated'], tolerance=0.0005)
            t_qm_simp, qm_simp = simplify_curve(result['t_min'], result['Qm_simulated'], tolerance=0.001)
            ax.plot(t_f_simp, f_simp, 'b-', linewidth=2, label='Degradation Factor F(t)')
            ax.set_xlabel("Time (min)")
            ...
            ax2 = ax.twinx()
            ax2.plot(t_qm_simp, qm_simp, 'r-', linewidth=2, alpha=0.7, label='Effective Capacity Qm(t)')
            ...
            fig, ax = plt.subplots(figsize=(6, 4))
            # 简化曲线
            t_temp_simp, temp_simp = simplify_curve(result['t_min'], result['T_celsius'], tolerance=0.5)
            ax.plot(t_temp_simp, temp_simp, 'orange', linewidth=2)
            ax.set_xlabel("Time (min)")
            ...
            # 简化曲线
            t_error_simp, error_simp = simplify_curve(result['t_min'], error, tolerance=0.2)
            t_up_simp, up_simp = simplify_curve(result['t_min'], result['Up_simulated'] * 1000, tolerance=0.5)
            ax.plot(t_error_simp, error_simp, 'g-', linewidth=1.5, label='SOC Error')
            ax.axhline(y=0, color='k', linestyle='--', linewidth=0.5)
            ax.fill_between(t_error_simp, error_simp, 0, alpha=0.2, color='green')
            ax.set_xlabel("Time (min)")
            ax.set_ylabel("SOC Error (%)", color='green')
            ax.tick_params(axis='y', labelcolor='green')
            
            ax2 = ax.twinx()
            ax2.plot(t_up_simp, up_simp, 'm-', linewidth=1.5, alpha=0.8, label='Polarization Voltage')
            ...
            fig, ax = plt.subplots(figsize=(6, 4))
            # 简化曲线
            t_p_meas_simp, p_meas_simp = simplify_curve(result['t_min'], result['P_measured'], tolerance=0.1)
            t_p_model_simp, p_model_simp = simplify_curve(result['t_min'], result['P_model'], tolerance=0.1)
            ax.plot(t_p_meas_simp, p_meas_simp, 'b-', linewidth=1, alpha=0.6, label='Measured Power')
            ax.plot(t_p_model_simp, p_model_simp, 'r-', linewidth=1.5, label='Model Power')
            P_rmse = np.sqrt(np.mean((result['P_model'] - result['P_measured'])**2))
            ax.set_xlabel("Time (min)")
            ...
            # 堆叠面积图
            ax.fill_between(t_plot, 0, P_cum1,
                           label='Screen', color='#ffa726', alpha=0.7)
            ax.fill_between(t_plot, P_cum1, P_cum2,
                           label='CPU', color='#66bb6a', alpha=0.7)
            ax.fill_between(t_plot, P_cum2, P_cum3,
                           label='Network', color='#42a5f5', alpha=0.7)
            ax.fill_between(t_plot, P_cum3, P_cum4,
                           label='GPS', color='#ef5350', alpha=0.7)
            
            # 总功率线
            ax.plot(t_plot, result['P_model'], 'k-', linewidth=1.5, label='Total Power')
