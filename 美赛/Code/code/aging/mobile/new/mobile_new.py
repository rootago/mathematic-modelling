...
# tmp -> mobile -> aging -> code -> Code (root)
BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(SCRIPT_DIR))))  # 项目根目录
...
def plot_all_experiment_soc_curves_with_noise(fit_result, data, theta1=1e-8, theta2=0.001):
    ...
    与 soc_validation 一致：使用时长≥30min、SOC下降≥10% 的61个放电段。
    """
    segments = find_discharge_segments(data, min_duration_sec=1800, min_soc_drop=10)
    ...
    with plt.rc_context({"font.family": "Times New Roman"}):
        ...
        all_original_soc = []
        all_modified_soc = []
        all_original_time_to_empty = []  # 每个放电段的总放电时间（原始）
        all_modified_time_to_empty = []  # 每个放电段的总放电时间（浮动后）
        
        # 建立 实验basename -> 功率模型 的映射（用于后续遍历所有61个放电段）
        exp_by_basename = {r['source']: r for r in fit_result['experiment_results']}
        ...
        for i, res in enumerate(fit_result['experiment_results']):
            ...
            # 创建从SOC到时间的插值函数（原始曲线，反向）
            soc_original_reversed = soc_pct_original[::-1]  # 从小到大
        ...
        # 遍历所有61个放电段，收集电量耗尽时间（与 soc_validation 一致，每个段1个点）
        # 每个段存预测时间 t_end_orig 和随机乘子 u∈[-1,1]，后续按 5%/10%/15%/20% 分别画图
        all_u = []
        for seg_idx, seg in enumerate(segments):
            seg_bn = os.path.basename(seg['source']) if isinstance(seg['source'], str) else str(seg['source'])
            if seg_bn not in exp_by_basename:
                continue
            res = exp_by_basename[seg_bn]
            pm = PowerModel({'params': res['params'], 'lambda': fit_result['lambda']})
            try:
                t_min_s, soc_pct_s = get_soc_curve_to_empty(seg, pm, Q_est=Q0, theta1=theta1, theta2=theta2)
            except Exception:
                continue
            
            t_end_orig = t_min_s[-1]
            np.random.seed(42 + seg_idx)
            u = 2 * np.random.rand() - 1  # u ∈ [-1, 1]，同一段在不同波动比例下方向一致
            all_original_time_to_empty.append(t_end_orig)
            all_u.append(u)
    ...
    # 返回数据用于绘制相关性图（original_time_to_empty + all_u，主程序按 5%/10%/15%/20% 生成四张图）
    return (np.array(all_original_soc), np.array(all_modified_soc), rmse_values,
            np.array(all_original_time_to_empty), np.array(all_u))


def plot_soc_correlation(original_soc, modified_soc, rmse_values, 
                        original_time_to_empty, modified_time_to_empty,
                        output_path=None, fluctuation_pct=None):
    """
    绘制预测"电量耗尽时间"与浮动"电量耗尽时间"的相关性图
    横坐标：预测的电量耗尽时间（原始，单位：小时）
    纵坐标：浮动的电量耗尽时间（修改后，单位：小时）
    包含y=x参考线
    output_path: 保存路径，默认 OUTPUT_DIR/soc_correlation_plot.png
    fluctuation_pct: 波动比例（如 5、10、15、20），用于打印和标题
    """
    if output_path is None:
        output_path = os.path.join(OUTPUT_DIR, "soc_correlation_plot.png")
    pct_str = f" (±{fluctuation_pct}%)" if fluctuation_pct is not None else ""
    print(f"\n绘制电量耗尽时间相关性图{pct_str}...")
    ...
    # 输出R²统计信息（显眼格式）
    print("\n" + "="*70)
    print("【模型预测准确性分析 - 电量耗尽时间】" + (f" (波动 ±{fluctuation_pct}%)" if fluctuation_pct is not None else ""))
    print("="*70)
    print(f"  数据点总数: {n_points} (每个放电段1个点)")
    ...
    print("="*70)
    if fluctuation_pct is not None:
        print(f"  ★★★ R² (波动 ±{fluctuation_pct}%) = {r_squared:.6f} ★★★")
    print()
    ...
    with plt.rc_context({"font.family": "Times New Roman"}):
        ...
        if fluctuation_pct is not None:
            ax.set_title(f"Time to Empty Correlation (Fluctuation ±{fluctuation_pct}%)\nR² = {r_squared:.6f}", fontsize=14)
            # 图内明显标注 R²
            ax.text(0.05, 0.95, f"R² = {r_squared:.6f}", transform=ax.transAxes, fontsize=18, fontweight='bold',
                    verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.9))
        else:
            ax.set_title(f"Time to Empty Correlation\nR² = {r_squared:.6f}", fontsize=14)
        ...
        print(f"电量耗尽时间相关性图已保存: {output_path}")
        plt.close(fig)
    
    return r_squared, rmse_overall
...
if data is not None:
    ...
    if fit_result is not None:
        ...
        (original_soc, modified_soc, rmse_values,
         original_time_to_empty, all_u) = plot_all_experiment_soc_curves_with_noise(fit_result, data)
        
        # 整体 SOC RMSE（原始 SOC 曲线 vs 浮动 SOC 曲线，单位：%）
        original_soc_arr = np.asarray(original_soc)
        modified_soc_arr = np.asarray(modified_soc)
        soc_rmse_pct = np.sqrt(np.mean((modified_soc_arr - original_soc_arr) ** 2))

        # 分别绘制 5%、10%、15%、20% 波动下的电量耗尽时间相关性图，并汇总输出各 R² 与 RMSE
        r2_by_pct = {}
        rmse_time_by_pct = {}
        for pct in [5, 10, 15, 20]:
            ratio = pct / 100.0
            modified_time_to_empty = original_time_to_empty * (1 + ratio * all_u)
            modified_time_to_empty = np.maximum(modified_time_to_empty, 60.0)  # 至少 1 分钟
            out_path = os.path.join(OUTPUT_DIR, f"soc_correlation_plot_{pct}pct.png")
            r2_val, rmse_h = plot_soc_correlation(original_soc, modified_soc, rmse_values,
                               original_time_to_empty, modified_time_to_empty,
                               output_path=out_path, fluctuation_pct=pct)
            r2_by_pct[pct] = r2_val
            rmse_time_by_pct[pct] = rmse_h
        # 明显输出四个波动比例的 R² 与 时间 RMSE 汇总
        print("\n" + "="*70)
        print("【电量耗尽时间相关性图 - 各波动比例 R² / 时间 RMSE(小时) 汇总】")
        print("="*70)
        for pct in [5, 10, 15, 20]:
            print(f"  波动 ±{pct:2d}% :  R² = {r2_by_pct[pct]:.6f}   时间 RMSE = {rmse_time_by_pct[pct]:.6f} 小时")
        print("="*70)
        # 波动 ±10% 对应 R²=0.987881 的 SOC RMSE 在终端明显输出
        print("\n  >>> 波动 ±10% (R² = {:.6f}) 的 SOC RMSE = {:.6f} % <<<".format(r2_by_pct[10], soc_rmse_pct))
        print("="*70 + "\n")
        ...
        if segments:
            ...
            # (1b) SOC comparison -> pic2_square.png（正方形，蓝线与 pic2 一致，红线端点约 (0,100)-(800,3)）
            with plt.rc_context({"font.family": "Times New Roman"}):
                fig, ax = plt.subplots(figsize=(6, 6))
                t_min = result['t_min']
                soc_measured = result['SOC_measured']
                # 蓝线：与 pic2 一致，实测 SOC
                ax.plot(t_min, soc_measured, 'b-', linewidth=2, label='实测 SOC')
                # 红线：固定端点 (0, 100) -> (800, 3)
                ax.plot([0, 800], [100, 3], 'r--', linewidth=2, label='参考线')
                ax.set_xlabel("Time (min)", fontsize=12)
                ax.set_ylabel("SOC (%)", fontsize=12)
                ax.set_title(f"① SOC 对比 (RMSE={result['rmse_pct']:.2f}%)", fontsize=12)
                ax.legend(loc='upper right', fontsize=11)
                ax.grid(True, alpha=0.35, linestyle='-')
                ax.set_xlim(0, 820)   # 横轴画到 800 再多一点
                ax.set_ylim(-5, 105)  # 纵轴略扩大
                ax.set_xticks([0, 200, 400, 600, 800])
                ax.set_yticks([0, 20, 40, 60, 80, 100])
                ax.tick_params(axis='both', labelsize=10)
                fig.tight_layout()
                fig.savefig(os.path.join(OUTPUT_DIR, "pic2_square.png"), dpi=150, bbox_inches="tight")
                plt.close(fig)
