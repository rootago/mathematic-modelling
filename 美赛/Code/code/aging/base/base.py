...
def fit_power_model(data):
    ...
    coef_names = ['C0_base', 'C1_screen', 'C2_bright', 'C3_cpu_usage_freq', 'C4_throughput', 'C5_wifi', 'C6_gps', 'C7_gps_quad']
    ...
    for source_file, group_df in data.groupby('source_file'):
        ...
        feature_names = ['x_base', 'x_screen_base', 'x_screen_bright', 'x_cpu_usage_freq', 
...
class PowerModel:
    ...
    def calc_power_from_row(self, row, return_components=False):
        ...
        features = [1.0, screen_on, screen_on * bright_norm, cpu_usage_freq, throughput, wifi_exp, gps_on, gps_quad]
        
        if return_components:
            # 返回各组件功率（含基础功耗）
            P_base = self.params[0]  # C0_base
            P_screen = self.params[1] * screen_on + self.params[2] * screen_on * bright_norm
            P_cpu = self.params[3] * cpu_usage_freq
...
if data is not None:
    ...
    if fit_result is not None:
        ...
        if segments:
            ...
            # ⑥ 各组件功率贡献（含基础功耗）-> pic7.png
            fig, ax = plt.subplots(figsize=(6, 4))
            
            # 计算累积功率（从基础功耗开始堆叠）
            P_cum0 = result['P_base']  # 基础功耗
            P_cum1 = P_cum0 + result['P_screen']
            P_cum2 = P_cum1 + result['P_cpu']
            P_cum3 = P_cum2 + result['P_network']
            P_cum4 = P_cum3 + result['P_gps']
            
            # 堆叠面积图
            ax.fill_between(result['t_min'], 0, P_cum0,
                           label='Base', color='#9e9e9e', alpha=0.7)
            ax.fill_between(result['t_min'], P_cum0, P_cum1,
            ...
            ax.set_xlabel("Time (min)")
            ax.set_ylabel("Power (W)")
            ax.set_title("Power Contribution by Component (with Base)", fontsize=11)
            ax.legend(loc='upper right', fontsize=9)
