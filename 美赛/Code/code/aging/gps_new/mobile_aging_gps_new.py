...
功率公式（从实测数据拟合，无基础功耗，多项式拟合）：
  P_total = P_screen + P_cpu + P_network + P_gps
  P_screen = C_base*screen + screen × (C_b1*bright + C_b2*bright² + C_b3*bright³)
  P_cpu = C_c1*cpu_feat + C_c2*cpu_feat² + C_c3*cpu_feat³
  P_net = (C_t1*tp + C_t2*tp² + C_t3*tp³) + (C_w1*wifi + C_w2*wifi² + C_w3*wifi³)
  P_gps = C_g1*gps_act + C_g2*gps_act² + C_g3*gps_act³
...
# SCRIPT_DIR: .../code/aging/gps_new
AGING_DIR = os.path.dirname(SCRIPT_DIR)                 # .../code/aging
CODE_DIR = os.path.dirname(AGING_DIR)                   # .../code
BASE_DIR = os.path.dirname(CODE_DIR)                    # .../Code (项目根目录)
...
def load_mobile_data_for_fitting(max_files=15):
    ...
    for folder in folders:
        ...
        for f in files:
            # 读取前几行检查 gps_status
            try:
                # 只读前 1000 行检查是否有 GPS 开启
                temp_df = pd.read_csv(f, nrows=1000, usecols=['gps_status'])
                if (temp_df['gps_status'] > 0).any():
                    has_gps = True
                    break
            except:
                continue
        ...
        if has_gps:
            gps_folders.append(folder)
        else:
            normal_folders.append(folder)
            
    print(f"发现 {len(gps_folders)} 个包含 GPS 开启数据的文件夹")
    
    # 组合文件夹列表：优先 GPS，然后是普通文件夹，总数限制
    target_folders = gps_folders + normal_folders
    target_folders = target_folders[:max_files]
    ...
    for folder in target_folders:
        ...
        for f in files:
            try:
                ...
                all_dfs.append(df)
                print(f"  加载: {folder} ({len(df)} 行) {'[GPS]' if folder in gps_folders else ''}")
...
def fit_power_model(data):
    ...
    coef_names = [
        'C_screen_base', 
        'C_bright_1', 'C_bright_2', 'C_bright_3',
        'C_cpu_1', 'C_cpu_2', 'C_cpu_3',
        'C_tp_1', 'C_tp_2', 'C_tp_3',
        'C_wifi_1', 'C_wifi_2', 'C_wifi_3',
        'C_gps_1', 'C_gps_2', 'C_gps_3'
    ]
    ...
    for source_file, group_df in data.groupby('source_file'):
        ...
        df['x_bright_1'] = df['screen_on'] * df['bright_norm']
        df['x_bright_2'] = df['screen_on'] * (df['bright_norm'] ** 2)
        df['x_bright_3'] = df['screen_on'] * (df['bright_norm'] ** 3)
        ...
        cpu_feat = cpu_usage_norm * freq_norm
        df['x_cpu_1'] = cpu_feat
        df['x_cpu_2'] = cpu_feat ** 2
        df['x_cpu_3'] = cpu_feat ** 3
        
        # 3. Network Throughput
        throughput_cols = ['wifi_rx', 'wifi_tx', 'mobile_rx', 'mobile_tx']
        existing_tp = [c for c in throughput_cols if c in df.columns]
        if existing_tp:
            tp_feat = df[existing_tp].sum(axis=1).clip(0, 1e7) / 1e6
        else:
            tp_feat = 0.0
        ...
        if 'wifi_intensity' in df.columns:
            ...
            wifi_feat = wifi_on * np.exp(-LAMBDA * wifi_int)
        else:
            wifi_feat = 0.0
            
        df['x_wifi_1'] = wifi_feat
        df['x_wifi_2'] = wifi_feat ** 2
        df['x_wifi_3'] = wifi_feat ** 3
        ...
        if 'gps_status' in df.columns:
            ...
            # 使用 gps_act_norm 的多项式
            df['x_gps_1'] = gps_on * gps_act_norm
            df['x_gps_2'] = gps_on * (gps_act_norm ** 2)
            df['x_gps_3'] = gps_on * (gps_act_norm ** 3)
        else:
            df['x_gps_1'] = 0.0
            df['x_gps_2'] = 0.0
            df['x_gps_3'] = 0.0
        ...
        feature_names = [
            'x_screen_base', 'x_bright_1', 'x_bright_2', 'x_bright_3',
            'x_cpu_1', 'x_cpu_2', 'x_cpu_3',
            'x_tp_1', 'x_tp_2', 'x_tp_3',
            'x_wifi_1', 'x_wifi_2', 'x_wifi_3',
            'x_gps_1', 'x_gps_2', 'x_gps_3'
        ]
    ...
    # 打印每个实验的结果及预测耗电时长
    print(f"\n成功拟合 {len(experiment_results)} 个独立实验 (多项式模型):")
    print("-" * 95)
...
class PowerModel:
    ...
    def calc_power_from_row(self, row, return_components=False):
        ...
        x_screen_base = screen_on
        x_bright_1 = screen_on * bright_norm
        x_bright_2 = screen_on * (bright_norm ** 2)
        x_bright_3 = screen_on * (bright_norm ** 3)
        ...
        wifi_feat = wifi_on * np.exp(-self.lam * wifi_int)
        
        x_wifi_1 = wifi_feat
        x_wifi_2 = wifi_feat ** 2
        x_wifi_3 = wifi_feat ** 3
        ...
        x_gps_1 = gps_on * gps_act_norm
        x_gps_2 = gps_on * (gps_act_norm ** 2)
        x_gps_3 = gps_on * (gps_act_norm ** 3)
        
        # Features vector matches coef_names order
        features = [
            x_screen_base, x_bright_1, x_bright_2, x_bright_3,
            x_cpu_1, x_cpu_2, x_cpu_3,
            x_tp_1, x_tp_2, x_tp_3,
        ...
        if return_components:
            ...
            P_screen = (self.params[0] * x_screen_base + 
                        self.params[1] * x_bright_1 + 
                        self.params[2] * x_bright_2 + 
                        self.params[3] * x_bright_3)
                        
            P_cpu = (self.params[4] * x_cpu_1 + 
                     self.params[5] * x_cpu_2 + 
                     self.params[6] * x_cpu_3)
                     
            P_network = (self.params[7] * x_tp_1 + 
                         self.params[8] * x_tp_2 + 
                         self.params[9] * x_tp_3 +
                         self.params[10] * x_wifi_1 + 
                         self.params[11] * x_wifi_2 + 
                         self.params[12] * x_wifi_3)
                         
            P_gps = (self.params[13] * x_gps_1 + 
                     self.params[14] * x_gps_2 + 
                     self.params[15] * x_gps_3)
