"""
2026 MCM Problem A：智能手机电池消耗建模 — 使用模拟退火算法拟合参数

与 main.py 的区别：使用模拟退火（Simulated Annealing）代替最小二乘法拟合 k, A, B。
...
- 拟合：模拟退火算法 min sum (U_obs - (Vocv(SOC)-I*r))^2，得 k,A,B。
...
if not os.path.exists(DYNAMIC_DATA_PATH):
    ...
    if t_fit is None:
        ...
        S_fit = S_fit[::step]
        print(f"模拟退火拟合数据点数: {len(t_fit)}, 时间范围: {t_fit[0]:.1f} ~ {t_fit[-1]:.1f} s")
        ...
        print("开始模拟退火优化...")
        # 使用 dual_annealing 进行全局优化
        result = dual_annealing(
            objective, 
            bounds=bounds,
            maxiter=5000,          # 最大迭代次数（增加到5000以获得更好的收敛）
        ...
        print(f"模拟退火拟合结果: k = {k:.6f}, A = {A:.6f}, B = {B:.6f}, 残差平方和 = {final_cost:.6f}")
        print(f"优化状态: {result.message}")
        print(f"函数评估次数: {result.nfev}")
        
        # 保存拟合参数
        with open(os.path.join(OUTPUT_DIR, "vocv_fit_params_sa.json"), "w", encoding="utf-8") as f:
            json.dump({
...
# ========== 3. 绘图 ==========
# ---------- 图1：fit_joint_U_sa.png（全部来自「实测数据」DST 放电段）----------
# 目的：看用模拟退火拟合的 k,A,B 算出的 U 模型 与 实测 U 是否一致；以及数据里的 SOC 随时间怎么变
if t_fit is not None:
    ...
    axes1[0].set_title("① 端电压：实测 vs 模型（模拟退火拟合好坏看两条线是否接近）", fontsize=10)
    ...
    fig1.suptitle("图1：模拟退火拟合结果（数据来源：data2 DST 放电段）", fontsize=12)
    fig1.tight_layout()
    fig1.savefig(os.path.join(OUTPUT_DIR, "fit_joint_U_sa.png"), dpi=150, bbox_inches="tight")
    print(f"Figure saved: {OUTPUT_DIR}/fit_joint_U_sa.png")

# ---------- 图2：battery_ode_sa.png（全部来自「数值模拟」，不是实测）----------
...
ax.set_title("④ 开路电压 Vocv 与 SOC 的关系（模拟退火拟合曲线）", fontsize=10)
ax.grid(True, alpha=0.5)
ax.set_facecolor("#fafafa")

fig2.suptitle("图2：ODE 数值模拟（起始 SOC=50%，恒功率 20W 放电 1h，模拟退火参数）", fontsize=12)
fig2.tight_layout()
fig2.savefig(os.path.join(OUTPUT_DIR, "battery_ode_sa.png"), dpi=150, bbox_inches="tight")
print(f"Figure saved: {OUTPUT_DIR}/battery_ode_sa.png")
